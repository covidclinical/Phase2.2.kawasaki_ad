...
if(filecheck){
  ...
  demo_raw <- demo_raw %>% 
    ...
    dplyr::filter(cohort == "AllAdm", 
                  ...
                  as.Date(admission_date, format = dateFormat) <= ymd('2024-01-01')) %>%
  ...
  clinical_raw <- clinical_raw %>% 
    dplyr::filter(cohort == "AllAdm",
                  ...
                  as.Date(calendar_date, format = dateFormat) <= ymd('2024-01-01')) %>%
...
demog_table <- toPlot_formated %>%
  ...
  dplyr::select( patient_num, sex, age_time_diagnosis, length_hospitalization,n_hospitalization ) %>%
  unique()
```
