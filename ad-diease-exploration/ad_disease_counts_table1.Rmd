...
icdCodes <- read.csv("../public-data/ICD_ad.csv", header = TRUE, colClasses = "character")
```
...
obs_disease_interest <- obs_filtered_inclusion_criteria %>%
  dplyr::mutate( threeDigitCode = substr( concept_code, 1, 3)) %>%
  dplyr::filter( threeDigitCode %in% icdCodes$ICD10_Code ) 
```
...
toPlot_formated <- obs_disease_interest %>%
  ...
  dplyr::left_join( icdCodes, by=c("threeDigitCode"="ICD10_Code"))
...
ggplot2::ggplot(data=toPlot_counts, aes(x=time_p, y=n), group = description) +
  ...
  ggplot2::labs(y = "Number of hospitalizations",
                x = "Date (by month)",
                title = "Number of monthly hospitazations") +
  ggplot2::theme_bw() +
